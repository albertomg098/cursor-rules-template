...
3. **Frontmatter** (at top of SKILL.md):
   ...
   globs: ["{{glob from Q3}}"]  # Only if auto-attach (omit if alwaysApply or manual)
   alwaysApply: true/false      # true if always-on, false otherwise (omit if using globs)
   ---
   ```
   ...
   **Important:** 
   ...
   - `alwaysApply` is OPTIONAL - only include if this is an always-on skill (true) or manual (false). If using `globs`, omit `alwaysApply`.
